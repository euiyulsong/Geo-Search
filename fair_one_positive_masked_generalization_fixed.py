...
def make_labels(df, mode, seed=42):
    ...
    elif mode == "one_positive_masked":
        # Label-efficiency / generalization experiment.
        #
        # Per query:
        #   - expose exactly ONE judged relevant POI (human_rel > 0) as positive
        #   - MASK all other judged relevant POIs from training
        #   - keep only true/random negatives (human_rel == 0) as negatives
        #
        # Evaluation is still performed on the untouched test set with
        # original human_rel in {0,1,2,3}.
        rng = np.random.default_rng(seed)
        parts = []

        for qid, group in out.groupby("qid", sort=False):
            relevant = group[group["human_rel"] > 0]
            negatives = group[group["human_rel"] == 0]

            if len(relevant) == 0 or len(negatives) == 0:
                continue

            chosen_idx = rng.choice(relevant.index.to_numpy())

            pos = group.loc[[chosen_idx]].copy()
            pos["train_label"] = 1

            neg = negatives.copy()
            neg["train_label"] = 0

            parts.append(pd.concat([pos, neg], axis=0))

        if not parts:
            raise ValueError("No usable query groups for one_positive_masked")
...
# ============================================================
# RIDGE REGRESSION
# Pointwise regression: every retained training row is learned.
# Random corpus negatives added by the dataset builder have train_label=0,
# so Ridge learns them as zero targets; there is no separate negative-loss step.
# ============================================================

def train_ridge(train, valid, alpha=1.0):
    X_train = train[WEIGHT_FEATURES].fillna(0).to_numpy(dtype=float)
    y_train = train["train_label"].to_numpy(dtype=float)

    model = Ridge(alpha=alpha)
    model.fit(X_train, y_train)

    print("\nRidge coefficients:")
    for f, c in zip(WEIGHT_FEATURES, model.coef_):
        print(f"  {f:20s}: {c:.6f}")
    print(f"  {'intercept':20s}: {model.intercept_:.6f}")
    return model

def evaluate_ridge(test, model, bootstrap):
    pred = test.copy()
    X = pred[WEIGHT_FEATURES].fillna(0).to_numpy(dtype=float)
    pred["score"] = model.predict(X)
    metrics = evaluate(pred, "score")
    low, high = bootstrap_ndcg(pred, "score", bootstrap)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Ridge L2 regularization strength"
    )
    ...
    print(
        "Ridge Regression vs LambdaRank"
    )
    ...
    for mode in modes:
        ...
        # ====================================================
        # CREATE TRAIN/VALID SUPERVISION ONLY
        # ====================================================
        # IMPORTANT:
        # - train/valid: expose one relevant positive/query and mask
        #   all other judged relevant POIs.
        # - test: NEVER mask. Keep the complete original test
        #   candidate set and original human_rel for evaluation.
        ...
        test = test_base.copy()
        test["train_label"] = test["human_rel"].astype(int)

        # Combined file is only for inspection/saving.
        # It is NOT used to construct the test set.
        labeled = pd.concat(
            [train, valid, test],
            axis=0,
            ignore_index=True
        ...
        print("\n" + "-" * 100)
        print("A. RIDGE REGRESSION")
        print("-" * 100)

        ridge_model = train_ridge(train, valid, alpha=args.ridge_alpha)
        ridge_metrics, ridge_pred = evaluate_ridge(
            test, ridge_model, args.bootstrap
        )

        print("\nRidge TEST (original human_rel 0-3)")
        for k, v in ridge_metrics.items():
            print(f"{k:15s}: {v:.4f}")

        ridge_pred.to_csv(
            f"fair_ridge_{mode}_predictions.csv", index=False
        )
        ...
        for f, c in zip(WEIGHT_FEATURES, ridge_model.coef_):
            row[f"w_{f}"] = c
    ...
    print(
        "[INFO] Skipped: this run contains only "
        "one_positive_masked, so there is no "
        "full_graded row to use as the retention baseline."
    )
    ...
    results.to_csv(
        "fair_one_positive_masked_results.csv",
        index=False
    )

    pivot.to_csv(
        "fair_one_positive_masked_ndcg5_pivot.csv"
    )
    ...
    for mode in modes:
        ...
        print(
            f"fair_ridge_{mode}_predictions.csv"
        )
