...
def make_labels(df, mode, seed=42):
    ...
    elif mode == "one_positive_generalization":
        # Generalization experiment:
        # reveal exactly ONE relevant example (human_rel > 0) per query.
        # All other rows are treated as 0 for training, including other relevant
        # judged POIs and random corpus negatives.
        #
        # IMPORTANT: evaluation still uses the original human_rel (0/1/2/3),
        # so success means the model learned from one labeled relevant POI and
        # ranked unseen relevant POIs highly.
        rng = np.random.default_rng(seed)
        out["train_label"] = 0

        for qid, group in out.groupby("qid", sort=False):
            positive_pool = group[group["human_rel"] > 0]
            if len(positive_pool) == 0:
                continue
            chosen = rng.choice(positive_pool.index.to_numpy())
            out.loc[chosen, "train_label"] = 1
...
def main():
    ...
    results.to_csv(
        "fair_one_positive_generalization_results.csv",
        index=False
    )

    pivot.to_csv(
        "fair_one_positive_generalization_ndcg5_pivot.csv"
    )
